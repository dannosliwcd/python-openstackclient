...
class ListUsage(command.Lister):
    ...
    def take_action(self, parsed_args):
        ...
        if not sdk_utils.supports_microversion(compute_client, "2.40"):
            usage_list = list(compute_client.usages(
                start.strftime(date_api_format), end.strftime(date_api_format),
                detailed=1))
        else:
            ...
            while marker:
                next_usage_list = list(compute_client.usages(
                    start.strftime(date_api_format),
                    end.strftime(date_api_format), detailed=1, marker=marker))
...
class ShowUsage(command.ShowOne):
    ...
    def take_action(self, parsed_args):
        ...
        usage = compute_client.get_usage(
            project, start.strftime(date_api_format),
            end.strftime(date_api_format))
